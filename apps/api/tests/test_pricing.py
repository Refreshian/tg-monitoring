...
def test_quote_picks_smallest_fitting_tariff() -> None:
    ...
    # 81 messages → Стартовый (10_000), 28% off 45500 → 32760 → 32800
    ...
    assert quote.quote_price_rub == 32_800
...
def test_quote_for_weekly_6444_uses_starter_plus() -> None:
    ...
    quote = quote_access_price(monthly, tariffs=tariffs, discount_ratio=0.28)
    assert quote is not None
    assert quote.tariff_name == "Стартовый плюс"
    assert quote.list_price_rub == 68_900
    assert quote.quote_price_rub == 49_600  # 68900 * 0.72 → 49608 → 49600
